...
import { IsBoolean, IsDate, IsNotEmpty, IsNumber, IsString, MaxLength, Min, MinDate } from "class-validator";
import { IsFutureDate } from "../decorator/date-validator.decorator";



export class CreateVoucerDto{
    @IsString()
    ...
    @IsNumber()
    @IsNotEmpty({message: "Jumlah voucer yang valid tidak boleh kosong!"})
